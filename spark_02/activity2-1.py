...
# Get hdfs prefix, to find full path
process = Popen(f'hdfs getconf -confKey fs.defaultFS', shell=True, stdout=PIPE, stderr=PIPE)
std_out, std_err = process.communicate()
hdfs_prefix = std_out.decode()[:-1]
print(hdfs_prefix)

# Get complete filenames
filenames = []
for file in list_of_file_names_with_full_address:
    filenames.append(hdfs_prefix+file)
print(filenames)
print("READ")
...
result.coalesce(1).write.csv(hdfs_prefix+"/user/arthurlima/actv2/run001")
print("PRINT")
